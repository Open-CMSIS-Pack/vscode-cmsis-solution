...
5. Read and apply [development principles](.agents/instructions/principles.md) only when following the plan or change skill. Do not use them for questions, reports, or other non-planning, non-change requests.
6. Local guidance has precedence over public guidance. If local and public guidance conflict, or local files conflict with each other, ask the user which rule to follow before proceeding.
