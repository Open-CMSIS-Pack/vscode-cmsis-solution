...
- Updates:
  - Added "Search in Solution Sources" to **CMSIS View** as magnifying glass icon, searching in the scope of the active target including files referenced from CMSIS packs and YAML files.
  - Added output file `.cmsis/tools-environment.yml` generated by the extension, logging tool versions and environment variables configured and contributed by the CMSIS extensions Solution, Debugger and SDS. This file can be used by AI agents and CI systems to replicate the tools setup.
  - Refined output file `.vscode/cmsis.json` to log the loaded `*.csolution.yml` file and active target. This helps AI agents to navigate in workspace folders with multiple solutions.
  - Made **Configuration Wizard** more error tolerant and added display of annotation errors with option to navigate to the error location in the editor view.
  - The Tree-View of the **CMSIS-View** decorates files and groups that are excluded from build using e.g. `for-context:`.

- Solved issues:
  - Create Solution workflow opens and edits wrong solution if multiple `*.csolution.yml` files are present in new solution directory.
