...
export const cbuildIdxFileFactory = makeFactory<CbuildIdxFile, ITreeItemFile>({
    ...
    cbuildFiles: (r) => new Map(r.activeContexts?.map(ctx => [ctx.projectName, cbuildFileFactory({
        outDir: ctx.projectPath ? `${path.dirname(ctx.projectPath)}/out` : undefined
    })]) ?? []),
    cbuildLoadResult: () => (ETextFileResult.Success),
    getContext: (r) => (projectName: string) => r.activeContexts?.find(ctx => ctx.projectName === projectName),
}, treeItemFileFactory);
