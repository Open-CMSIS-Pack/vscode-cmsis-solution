/**
 * Copyright 2024-2026 Arm Limited
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     https://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

import { ParseError, parse as parseJson, printParseErrorCode } from 'jsonc-parser';
import * as YAML from 'yaml';
import { ErrorList, IErrorList } from './error-list';

export interface ITextParser extends IErrorList {
    /**
     * Parse string to designated object
     * @param content string to parse
     * @returns created object or empty
     */
    parse(content: string): object;

    /**
     * Converts supplied object to string of dedicated format
     * @param obj object to stringify
     * @return string
     */
    stringify(obj?: unknown): string;
}

// base class for parsers, can be mocked
export class TextParser extends ErrorList implements ITextParser {
    parse(_content: string): object {
        return {};
    }
    stringify(_obj?: unknown): string {
        return '';
    }
}

export class SimpleJsonParser extends ErrorList implements ITextParser {
    parse(text: string): object {
        this.clearErrors();
        const parseErrors: ParseError[] = [];
        let result = parseJson(text, parseErrors, { disallowComments: false, allowTrailingComma: true, allowEmptyContent: true });
        if (parseErrors.length > 0) {
            for (const err of parseErrors) {
                this.addError(
                    `${printParseErrorCode(err.error)} (offset: ${err.offset}, length: ${err.length})`
                );
            }
            result = {};
        }
        return result;
    };

    stringify(obj?: unknown): string {
        return obj ? JSON.stringify(obj, null, 4) : '';
    }
}

export class SimpleYamlParser extends ErrorList implements ITextParser {
    parse(content: string): object {
        this.clearErrors();
        try {
            return YAML.parse(content);
        } catch (e) {
            this.addError(e instanceof Error ? e.message : String(e));
        }
        return {};
    };

    stringify(obj: unknown): string {
        return obj ? YAML.stringify(obj, null, 4) : '';
    }
}

